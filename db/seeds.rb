...
CATALOG = {
  ...
  "Globex Tecnología" => {
    minimum_purchase_cents: 100_000,
    products: [
  ...
  "Wayne Suministros" => {
    minimum_purchase_cents: 150_000,
    products: [
...
CATALOG.each do |supplier_name, attrs|
  supplier = Supplier.find_or_create_by!(name: supplier_name)
  supplier.update!(minimum_purchase_cents: attrs[:minimum_purchase_cents])

  attrs[:products].each do |product_attrs|
    product = supplier.products.find_or_initialize_by(name: product_attrs[:name])
    product.update!(price_cents: product_attrs[:price_cents], stock: product_attrs[:stock])
  end
end
